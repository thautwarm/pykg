...
def create_metadata_from_project(url_o: ClassifiedUrl, proj: Project):
    ...
    for each in proj.locals:
        ...
        locals[each.uncomment.package.uncomment] = classify_url(url.link.uncomment, branch and branch.uncomment)

    return locals, Metadata(
        proj.name,
        proj.authors,
        [
            Commented(
                [], Dist(version=proj.version, url=None, deps=proj.deps)
            )
        ],
    )
