...
from reflect import parser_wrap

from reflect.pretty_doc import RenderOptions
...
if typing.TYPE_CHECKING:
    ...
    def reflect(cls: type, scope: dict | None = None):
        """setup reflection for dataclasses"""
        if not is_dataclass(cls):
            raise TypeError(
                f"{reflect.__name__} is not applicable to non-dataclass {cls}!"
            )
