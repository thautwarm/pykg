...
@cache
def get_git_repo(classified_url: ClassifiedUrl):
    return Git(classified_url)
...
class Git:
    """
    virtual git.
    if provider is specified, it is a cloud host;
    otherwise, it is a local repo.
    """

    def __init__(self, classified_url: ClassifiedUrl):
        ...
        if isinstance(classified_url, LocalProjUrl):
            self.local_git_dir = classified_url.local_git_directory()
    ...
    def _setup_git_repo(self, classified_url: CloudGitRepoUrl):
        ...
        def update_func(pykg_module_path):
            ...
            for path, newfile in results:
                path_o = pykg_module / self.local_git_dir / path
                if newfile is None:
