...
def construct_token(token_id, lexeme, line, col, span, offset, file):
    if token_id == -1:
        return token("EOF", "")
    return token(
        tokenmaps[token_id], lexeme, offset, line, col, None, None, span + offset
    )
...
with (__import__("pathlib").Path(__file__).parent / "comfig.lark").open(
    encoding="utf8"
) as __0123fx9:
