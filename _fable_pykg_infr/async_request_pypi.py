...
from _fable_pykg.src.comp import mk_version, mk_specifier
...
def require_python_package_versions_and_deps(
                ...
                if spec.operator == "==":
                    ...
                    if match_group["micro"] is None:
                        specifiers.extend(
                            [
                                mk_specifier(comp.GE, mk_version(major, minor, 0)),
                                mk_specifier(comp.LT, mk_version(major + 1, 0)),
                            ]
                        )
                        continue
                    micro = int(match_group["micro"])
    ...
    if sync:
        results = []
        for ver in versions:
            typed_ver = get_typed_version(ver)
            package = yield from task(ver)
            if package is None:
                continue
            results.append((typed_ver, package))
        return results
    else:
        packages = yield from gather_with_limited_workers(
            list(map(task, versions)), nworkers=nworkers
        )
        
        typed_versions = map(get_typed_version, versions)
        return [(get_typed_version(ver), package) for ver, package in zip(versions, packages) if package is not None]
